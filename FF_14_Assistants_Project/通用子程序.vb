Public Class Common
    Public Delegate Sub wt(ByVal type As Integer, ByVal Text As String, ByVal Level As Integer)
    Public Shared F As New Form_Main
    Public Overloads Shared Sub Do_Output(ByVal type As Integer, ByVal Text As String, ByVal Level As Integer)
        ...
        If Level <= My.Settings.Output_Level Then '该消息输出等级小于等于当前设定的输出等级时才输出
            ...
            F.TextBox_Output.Text = (F.TextBox_Output.Text &
                "[" & TimeOfDay & "]" & ":" & "[" & TypeText & "]" & Text & vbCrLf) '输出文本

            F.TextBox_Output.SelectionStart = Len(F.TextBox_Output.Text)
            F.TextBox_Output.ScrollToCaret() '此处将输出文本框定位到末尾位置
    ...
    Public Shared Sub Output(ByVal type As Integer, ByVal Text As String, ByVal Level As Integer)
        Dim d As New wt(AddressOf Common.Do_Output)
        Form_Main.CreateGraphics()
        Form_Main.Invoke(d, type, Text, Level)
    End Sub
